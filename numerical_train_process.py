...
class PPOAgent:
    ...
    def update(self):
        ...
        node_features = torch.stack([self.compute_node_features(state) for state in states])
        global_infos = torch.stack([self.compute_global_info(state) for state in states])
        ...
        nxt_node_features = torch.stack([self.compute_node_features(next_state) for next_state in next_states])
        nxt_global_infos = torch.stack([self.compute_global_info(next_state) for next_state in next_states])
        ...
        advantages = self.compute_advantage(td_delta, dones)
...
def load_data_and_create_envs(file_path, num, max_time):
    """
    读取文件夹中的数据并创建环境实例。
...
if __name__ == "__main__":
    ...
    num_projects = 30
    max_time = 100
    env_list = load_data_and_create_envs(file_path, 1, max_time)
    ...
    file_path1 = 'PPO_Agent_model_output_30.pth'
    ppo.save(file_path1)